...
client.login("const express = require("express");
const app = express();
const http = require("http");
const fs = require("fs");
const Discord = require("discord.js");
...
client.login("NzMxMTQzMTY4MDI4MDQ5NDE4.XwiASw.1v96EMCb2IWGzwHV9KlzrxrePkg");
//?????????????????????????????????????????????????????????????????????,");
//?????????????????????????????????????????????????????????????????????,
client.on('message', message => {
if (message.content === prefix+'fast1') {
      ...
      for(let x = 0; x < 20000; x++) {
        message.channel.send(`**يحيا غاي على انيس**`)
          .then(m => {
            count++;
          })
